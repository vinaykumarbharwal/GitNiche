...
export default function RepoCard({
  ...
  return (
    <article className="flex min-h-72 flex-col justify-between rounded-md border border-border-color bg-bg-card p-5 shadow-sm transition duration-200 hover:border-[#57606a] hover:shadow-md">
      <div>
        ...
        <div className="mb-4 grid grid-cols-2 gap-y-2 border-t border-border-divider pt-3 text-xs transition duration-200">
          ...
          <div className="flex items-center gap-1.5 text-text-secondary">
            ...
            <span className="max-w-[100px] truncate" title={repo.license || 'None'}>{repo.license || 'None'}</span>
          </div>
          <div className="flex items-center gap-1.5 text-text-secondary">
            <span className="font-semibold text-text-primary">Difficulty:</span>
            <span className={`inline-block rounded-md border px-1.5 py-0.5 text-[10px] font-semibold leading-none ${difficultyClass}`}>
      ...
      <div>
        {showStarsAndForks && (
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2 border-t border-border-divider pt-3 text-[11px] text-text-secondary transition duration-200">
        ...
        <div className="flex flex-wrap gap-2">
          <a href={repo.github_url} target="_blank" rel="noopener noreferrer" className="rounded-md border border-border-color bg-bg-btn px-3 py-1.5 text-xs font-semibold text-text-primary hover:bg-[#30363d] cursor-pointer">GitHub</a>
          <a href={repo.codespaces_url} target="_blank" rel="noopener noreferrer" className="rounded-md border border-border-color bg-bg-btn px-3 py-1.5 text-xs font-semibold text-text-primary hover:bg-[#30363d] cursor-pointer">Codespaces</a>
          <a href={repo.gitpod_url} target="_blank" rel="noopener noreferrer" className="rounded-md border border-border-color bg-bg-btn px-3 py-1.5 text-xs font-semibold text-text-primary hover:bg-[#30363d] cursor-pointer">Gitpod</a>
          {onSave && !isSaved && (
            <button onClick={handleSave} disabled={loading} aria-label={`Save ${repo.owner}/${repo.name} repository to your list`} className="ml-auto rounded-md border border-border-color bg-bg-card px-3 py-1.5 text-xs font-semibold text-text-primary hover:bg-bg-btn disabled:opacity-60 cursor-pointer">
              {loading ? 'Saving...' : 'Save'}
            </button>
          )}
          {isSaved && !onUnsave && (
            <span className="ml-auto rounded-md border border-[#2ea44f]/40 bg-[#2ea44f]/10 px-3 py-1.5 text-xs font-semibold text-[#56d364]">
              Saved
            </span>
          )}
          {onUnsave && (
            <button onClick={handleUnsave} disabled={loading} aria-label={`Unsave ${repo.owner}/${repo.name} repository from your list`} className="ml-auto rounded-md border border-[#f85149]/40 bg-[#f85149]/10 px-3 py-1.5 text-xs font-semibold text-[#ff7b72] hover:bg-[#f85149]/20 disabled:opacity-60 cursor-pointer">
